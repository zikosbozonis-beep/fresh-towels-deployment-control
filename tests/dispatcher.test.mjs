          ...
          async run() {
            if (sql.startsWith("INSERT")) {
              ...
              database.rows.set(requestId, {
                controllerSha: controllerShaValue,
                digest,
                jti,
                nonce,
                operation,
            ...
            if (sql.startsWith("UPDATE")) {
              ...
              if (sql.includes("SET state = 'executing'")) {
                ...
                const row = database.rows.get(requestId);
                if (!row || row.state !== "dispatched" || row.digest !== digest)
                  return { meta: { changes: 0 }, success: true };
                for (const candidate of database.rows.values()) {
                  if (
                    candidate.executorJti === executorJti ||
                    candidate.controllerRunId === controllerRunId
                  )
                    throw new Error("D1 UNIQUE constraint failed");
                }
                Object.assign(row, {
...
async function fixture(claimOverrides = {}, operation = "production-release") {
  ...
  const fetcher = async (url, options = {}) => {
    ...
    if (String(url).includes("/access_tokens"))
      return Response.json({
        repositories: [{ id: 2002 }],
        token: "synthetic-installation-token-value",
      });
    if (String(url).endsWith("/dispatches")) return new Response(null, { status: 204 });
    throw new Error("unexpected network request");
  };
  const makeRequest = () =>
    new Request("https://dispatcher.example/v1/dispatch", {
...
test("valid GitHub OIDC evidence dispatches with a down-scoped public-only App token", async () => {
  ...
  assert.equal(
    value.applicationEnvironment.DISPATCH_STATE.rows.get("11111111-1111-4111-8111-111111111111")
      .state,
    "dispatched",
  );
});

test("safe dispatcher diagnostics reveal only an allowlisted stage or claim name", () => {
  assert.equal(
    safeRejectionReason(new Error("OIDC claim workflow_sha differs")),
    "oidc_claim_workflow_sha_differs",
  );
  assert.equal(
    safeRejectionReason(new Error("App installation token issuance failed")),
    "app_token_issuance_failed",
  );
  assert.equal(
    safeRejectionReason(new Error("attacker supplied secret-like text")),
...
test("altered caller identity and altered signatures fail before App token issuance", async () => {
  ...
  const rejectedClaim = await handleDispatch(
    wrongWorkflow.httpRequest,
    wrongWorkflow.applicationEnvironment,
    wrongWorkflow.fetcher,
    now,
  );
  ...
  const invalidRequest = new Request("https://dispatcher.example/v1/dispatch", {
    body: JSON.stringify(body),
    headers: { authorization: `Bearer ${changed}`, "content-type": "application/json" },
    method: "POST",
  });
  const rejectedSignature = await handleDispatch(
    invalidRequest,
    tampered.applicationEnvironment,
    tampered.fetcher,
    now,
  );
...
test("stale evidence and unexpected public input fail without network activity", async () => {
  ...
  const injected = new Request("https://dispatcher.example/v1/dispatch", {
    body: JSON.stringify({ command: "anything", releaseRequestBase64: value.encoded }),
    headers: { authorization: `Bearer ${value.token}`, "content-type": "application/json" },
    method: "POST",
  });
  assert.equal(
    (await handleDispatch(injected, value.applicationEnvironment, value.fetcher, now)).status,
    401,
  );
});
...
test("ambiguous provider failure consumes evidence permanently", async () => {
  ...
  assert.equal(
    (await handleDispatch(value.makeRequest(), value.applicationEnvironment, failingFetcher, now))
      .status,
    401,
  );
  assert.equal(
...
test("protected executor OIDC creates one atomic execution claim only", async () => {
  ...
  const makeExecutionRequest = () =>
    new Request("https://dispatcher.example/v1/execute-claim", {
...
test("the same protected run records one hash-only terminal execution receipt", async () => {
  ...
  const executorToken = await jwt(value.oidcKeys.privateKey, executorClaims());
  const requestFor = (path, extra = {}) =>
    new Request(`https://dispatcher.example${path}`, {
  ...
  assert.equal(
    (
      await handleDispatch(
        requestFor("/v1/execute-finish", { outcome: "executed", providerReceiptSha256: receipt }),
        value.applicationEnvironment,
        value.fetcher,
        now,
      )
  ...
  const row = value.applicationEnvironment.DISPATCH_STATE.rows.get(
